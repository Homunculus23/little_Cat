$(".activate_reset").on("click", ()=>{
    ...
    if (determine_reset) window.onbeforeunload = ()=>{
...
$(".activate_delete").on("click", ()=>{
    ...
    if (number_activate_delete % 2 === 0) {
        delete_confirm = window.confirm(`
            ...
            再次点击回收站或刷新网页，退出删除状态。
        `);
        if (delete_confirm) number_activate_delete++;
    }
    if (delete_confirm) {
        $icon_wrapper.find(".icon_close").remove();
        hashMap.forEach((node, index)=>{
...
$(document).on("keypress", (e)=>{
    const { key  } = e;
    for(let i = 0; i < hashMap.length; i++)if (hashMap[i].shortcuts === key) window.open(hashMap[i].url);
});
